...
module srcnn88_line_buffer #(
    ...
    // Input pixel stream.
    // The stream must include zero padding:
    //   - 1 top zero row
    //   - 1 left/right zero pixel per image row
    //   - 1 bottom zero row
    ...
    // Scan rows shift horizontally during RUN so that the active 3x3 window
    // is always located at the fixed left edge of each row.
    ...
    always @(posedge i_clk or negedge i_rstn) begin
        if (!i_rstn) begin
            ...
            // Load one pixel into the row selected by r_cnt.
                ...
                if (r_cnt == (LINE_WIDTH * KERNEL_SIZE) - 1) begin
                    r_scan[0] <= r_line[0];
                    r_scan[1] <= r_line[1];
                    r_scan[2] <= {r_line[2][LINE_BITS-DATA_WIDTH-1:0], i_input_data};
                end
            ...
            // Move to the next output column after the current column's
            // computation has been issued.
    ...
    always @(*) begin
        o_window_00 = r_scan[0][((LINE_WIDTH - 1) * DATA_WIDTH) +: DATA_WIDTH];
        o_window_01 = r_scan[0][((LINE_WIDTH - 2) * DATA_WIDTH) +: DATA_WIDTH];
        o_window_02 = r_scan[0][((LINE_WIDTH - 3) * DATA_WIDTH) +: DATA_WIDTH];

        o_window_10 = r_scan[1][((LINE_WIDTH - 1) * DATA_WIDTH) +: DATA_WIDTH];
        o_window_11 = r_scan[1][((LINE_WIDTH - 2) * DATA_WIDTH) +: DATA_WIDTH];
        o_window_12 = r_scan[1][((LINE_WIDTH - 3) * DATA_WIDTH) +: DATA_WIDTH];

        o_window_20 = r_scan[2][((LINE_WIDTH - 1) * DATA_WIDTH) +: DATA_WIDTH];
        o_window_21 = r_scan[2][((LINE_WIDTH - 2) * DATA_WIDTH) +: DATA_WIDTH];
        o_window_22 = r_scan[2][((LINE_WIDTH - 3) * DATA_WIDTH) +: DATA_WIDTH];
    end
