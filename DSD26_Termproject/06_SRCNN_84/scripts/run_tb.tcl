...
# Project     : SRCNN_84
# File        : run_tb.tcl
# Purpose     : Non-project Vivado batch simulation flow for tb_SRCNN_84.
#
# Usage:
#   vivado -nolog -nojournal -notrace -mode batch \
#     -source ./06_SRCNN_84/scripts/run_tb.tcl
#
# Useful options:
...
#   - Temporary simulation files are isolated under 06_SRCNN_84/sim_work/run_*.
#============================================================================
...
foreach arg $argv {
    switch -- $arg {
        ...
        "--help" {
            puts "SRCNN_84 batch simulation"
            puts ""
            puts "Usage:"
            puts "  vivado -nolog -nojournal -notrace -mode batch -source ./06_SRCNN_84/scripts/run_tb.tcl"
            puts ""
            puts "Options:"
            puts "  --keep    Keep the current 06_SRCNN_84/sim_work/run_* directory after the run."
...
set rtl_files [list \
    [file join $rtl_dir "srcnn84_simple_dual_port_bram.v"] \
    [file join $rtl_dir "srcnn84_controller.v"] \
    [file join $rtl_dir "srcnn84_line_buffer.v"] \
    [file join $rtl_dir "srcnn84_pad_insert.v"] \
    [file join $rtl_dir "srcnn84_pe.v"] \
    [file join $rtl_dir "srcnn84_post_process.v"] \
    [file join $rtl_dir "srcnn84_weight_bias_rom.v"] \
    [file join $rtl_dir "srcnn84_top.v"] \
]

set tb_file  [file join $tb_dir "tb_SRCNN_84.v"]
set top_name "tb_SRCNN_84"
...
print_banner "SRCNN_84 batch simulation start"
...
if {[catch {
    ...
    print_banner "SRCNN_84 simulation FAILED"
    puts "Reason:"
    puts "  $result"
    puts ""
...
if {$keep_work} {
    print_banner "SRCNN_84 simulation PASSED"
