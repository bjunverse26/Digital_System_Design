...
module srcnn42_controller #(
    ...
    output reg  [$clog2(IMAGE_WIDTH + KERNEL_SIZE)-1:0] o_line_input_col,
    output reg                           o_line_shift,

    // PE enable. Asserted once per output pixel or 2-pixel group.
    output reg                           o_mac_en,

    // Shift the line-buffer window after the current output column/group is issued.
    ...
    reg                          r_issue_valid_d;
    reg                          r_issue_zero_d;
    reg [LINE_CNT_WIDTH-1:0]     r_issue_col_d;
    ...
    wire w_last_layer;
    wire w_last_image;
    wire w_packed_input_layer;
    wire w_fill_row_done;
    wire w_window_shift;
    ...
    // Layer 1/2 issue two adjacent output columns per MAC cycle.
    // Layer 3 still writes one final output pixel per cycle.
    assign w_run_row_done       = (o_layer == LAYER_3) ? (r_run_col >= IMAGE_WIDTH - 1) :
                                                         (r_run_col >= IMAGE_WIDTH - 2);
    ...
    assign w_packed_input_layer = (o_layer != LAYER_1);

    // Layer 2/3 read two horizontally adjacent pixels as one activation word.
    assign w_fill_row_done      = w_packed_input_layer ? (r_stream_col >= IMAGE_WIDTH - 1) :
                                                         (r_stream_col >= LINE_WIDTH - 1);
    ...
    always @(*) begin
        ...
        case (r_state)
            ...
            S_RUN: begin
                // Issue one MAC per output column or 2-pixel group.
    ...
    always @(posedge i_clk or negedge i_rstn) begin
        if (!i_rstn) begin
            ...
            r_issue_zero_d     <= 1'b0;
            r_issue_col_d      <= {LINE_CNT_WIDTH{1'b0}};
            ...
            o_line_input_col   <= {$clog2(IMAGE_WIDTH + KERNEL_SIZE){1'b0}};
            o_line_shift       <= 1'b0;
            o_mac_en           <= 1'b0;
            o_done             <= 1'b0;
        end else begin
            ...
            o_line_input_col   <= r_issue_col_d[$clog2(IMAGE_WIDTH + KERNEL_SIZE)-1:0];
            r_issue_valid_d    <= 1'b0;
            r_issue_zero_d     <= 1'b0;
            r_issue_col_d      <= {LINE_CNT_WIDTH{1'b0}};
            ...
            case (r_state)
                ...
                S_PRELOAD: begin
                    ...
                    r_stream_col   <= w_packed_input_layer ? {{(LINE_CNT_WIDTH-1){1'b0}}, 1'b1} :
                                                              {LINE_CNT_WIDTH{1'b0}};
                    r_fill_row_cnt <= {FILL_CNT_WIDTH{1'b0}};
                    r_fill_issued  <= 1'b0;
                ...
                S_FILL: begin
                    if (!r_fill_issued) begin
                        ...
                        if (w_fill_row_done) begin
                            ...
                            r_stream_col <= w_packed_input_layer ? (r_stream_col + 2'd2) :
                                                                  (r_stream_col + 1'b1);
                        end
                    end
                end
                ...
                S_RUN: begin
                    // Enable MAC once per output column or 2-pixel group.
                    o_mac_en <= 1'b1;

                    if (w_run_row_done) begin
                        r_run_col <= {($clog2(IMAGE_WIDTH)+1){1'b0}};
                        ...
                        r_run_col <= (o_layer == LAYER_3) ? (r_run_col + 1'b1) :
                                                            (r_run_col + 2'd2);
                    end
                end
