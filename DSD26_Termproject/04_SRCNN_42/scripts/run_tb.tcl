...
# Project     : SRCNN_42
# File        : run_tb.tcl
# Purpose     : Non-project Vivado batch simulation flow for tb_SRCNN_42.
#
# Usage:
#   vivado -nolog -nojournal -notrace -mode batch \
#     -source ./04_SRCNN_42/scripts/run_tb.tcl
#
# Useful options:
...
#   - Temporary simulation files are isolated under 04_SRCNN_42/sim_work/run_*.
#============================================================================
...
foreach arg $argv {
    switch -- $arg {
        ...
        "--help" {
            puts "SRCNN_42 batch simulation"
            puts ""
            puts "Usage:"
            puts "  vivado -nolog -nojournal -notrace -mode batch -source ./04_SRCNN_42/scripts/run_tb.tcl"
            puts ""
            puts "Options:"
            puts "  --keep    Keep the current 04_SRCNN_42/sim_work/run_* directory after the run."
...
set rtl_files [list \
    [file join $rtl_dir "srcnn42_simple_dual_port_bram.v"] \
    [file join $rtl_dir "srcnn42_line_buffer.v"] \
    [file join $rtl_dir "srcnn42_pe.v"] \
    [file join $rtl_dir "srcnn42_post_process.v"] \
    [file join $rtl_dir "srcnn42_weight_bias_rom.v"] \
    [file join $rtl_dir "srcnn42_controller.v"] \
    [file join $rtl_dir "srcnn42_top.v"] \
]

set tb_file  [file join $tb_dir "tb_SRCNN_42.v"]
set top_name "tb_SRCNN_42"
...
print_banner "SRCNN_42 batch simulation start"
...
if {[catch {
    ...
    print_banner "SRCNN_42 simulation FAILED"
    puts "Reason:"
    puts "  $result"
    puts ""
...
if {$keep_work} {
    print_banner "SRCNN_42 simulation PASSED"
